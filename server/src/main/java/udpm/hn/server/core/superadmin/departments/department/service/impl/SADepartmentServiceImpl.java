package udpm.hn.server.core.superadmin.departments.department.service.impl;
...
import udpm.hn.server.core.superadmin.departments.department.repository.SADepartmentExtendRepository;
...
@Slf4j
public class SADepartmentServiceImpl implements SADepartmentService {

    private final SADepartmentExtendRepository SADepartmentExtendRepository;
    ...
    public ResponseObject<?> getAllDepartment(FindDepartmentsRequest request) {
        ...
        Page<DDepartmentResponse> departmentResponses = SADepartmentExtendRepository.getAllDepartmentByFilter(pageable, request);
    ...
    public ResponseObject<?> getDetailDepartment(String id) {
        return new ResponseObject<>(SADepartmentExtendRepository.getDetailDepartment(id), HttpStatus.OK, "Lấy thành công bộ môn");
    }

    @Override
    public ResponseObject<?> getDepartmentsManagedByStaff(String staffId) {
        return new ResponseObject<>(SADepartmentExtendRepository.getDepartmentsManagedByStaff(staffId),  HttpStatus.OK, "Lấy thành công danh sách bộ môn theo trưởng môn");
    }
    ...
    public ResponseObject<?> addDepartment(@Valid CreateUpdateDepartmentRequest request) {
        ...
        boolean checkExistName = SADepartmentExtendRepository.existsByName(request.getDepartmentName().trim());
        boolean checkExistCode = SADepartmentExtendRepository.existsByCode(code);
        ...
        this.SADepartmentExtendRepository.save(department);
    ...
    public ResponseObject<?> updateDepartment(@Valid CreateUpdateDepartmentRequest request, String id) {
        ...
        Optional<Department> departmentOptional = SADepartmentExtendRepository.findById(id);
        ...
        if (!updateDepartment.getCode().trim().equalsIgnoreCase(code)) {
            if (SADepartmentExtendRepository.existsByCode(code)) {
                return new ResponseObject<>(null, HttpStatus.NOT_ACCEPTABLE, "Mã bộ môn đã tồn tại: " +
                        code);
            }
        }
        if (!updateDepartment.getName().trim().equalsIgnoreCase(request.getDepartmentName().trim())) {
            if (SADepartmentExtendRepository.existsByName(request.getDepartmentName().trim())) {
                return new ResponseObject<>(null, HttpStatus.NOT_ACCEPTABLE, "Tên bộ môn đã tồn tại: " +
        ...
        this.SADepartmentExtendRepository.save(updateDepartment);
    ...
    public ResponseObject<?> deleteDepartment(String id) {
        ...
        this.SADepartmentExtendRepository.save(deleteDepartment);
    ...
    public ResponseObject<?> synchronize() {
        ...
        try {
            ...
            List<Department> departments = SADepartmentExtendRepository.findAll();
    ...
    private void syncDepartment(Department department, DepartmentResponse departmentResponse) {
        ...
        if (department == null) {
            ...
            Optional<Department> departmentOptional = SADepartmentExtendRepository.findDepartmentByDepartmentIdentityId(
        ...
        SADepartmentExtendRepository.save(postDepartment);
    }
