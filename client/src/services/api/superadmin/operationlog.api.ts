...
import { Ref } from "vue";
import { PREFIX_API_SUPER_ADMIN_OPERATION_LOG, PREFIX_API_SUPER_ADMIN_USER_ACTIVITY } from "@/constants/url";
...
export const getOperationLogs = async (params: Ref<ParamsGetOperationLogs>) => {
    const res = (await request({
      url: `${PREFIX_API_SUPER_ADMIN_OPERATION_LOG}`,
      method: "GET",
      params: params.value,
    })) as AxiosResponse<
...
export const getUserActivityLog = async (params: Ref<ParamsGetUserActivityLog>) => {
  const res = (await request({
    url: `${PREFIX_API_SUPER_ADMIN_USER_ACTIVITY}`,
    method: "GET",
    params: params.value,
  })) as AxiosResponse<
...
export const getDetailOperationLog = async (operationLogId: string | null) => {
    const res = (await request({
      url: `${PREFIX_API_SUPER_ADMIN_OPERATION_LOG}/${operationLogId}`,
      method: "GET",
