<template>
    <a-dropdown :trigger="['click']" v-model:open="visible">
        <a-badge :count="tot" :overflow-count="5">
            ...
            <a-menu class="min-w-96">
                ...
                <div class="max-h-[480px] overflow-auto">
                    <div v-for="item in notificationData" :key="item.id">
                        <a-menu-item :class="item.status ? '' : 'hover:bg-blue-200 bg-blue-100'"
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <div class="size-2 rounded-full bg-blue-700" v-if="!item.status"></div>
                            </div>
                        </a-menu-item>
                        <a-menu-divider />
                    </div>
                ...
                <a-menu-item v-if="notificationData.length > 5">
                    <div class="flex">
                        <a-button :loading="isLoading || isFetching" class="m-auto" @click="handleShowMoreNotification">Hiển thị thêm thông báo</a-button>
                    </div>
                </a-menu-item>
            </a-menu>
        </template>
...
import { NotificationResponse, ParamsGetNotification, ParamsReadNotification } from '@/services/api/notification/notification.api';
import { useGetCountNotifications, useGetNotifications, useReadNotification } from '@/services/service/notification/notification.action';
...
const { data: listNotificationData, refetch: refetchNotificationData, isLoading, isFetching } = useGetNotifications(params, {
    refetchOnWindowFocus: false,
    placeholderData: keepPreviousData,
})

const { data: totalNotificationData, refetch: refetchCountNotification } = useGetCountNotifications(params, {
    refetchOnWindowFocus: false,
    placeholderData: keepPreviousData,
})
...
const notificationData = computed(() => listNotificationData.value?.data.data || []);
const tot = computed(() => totalNotificationData.value?.data || null);

onMounted(() => {
    if (props.keyWord === 'GIANG_VIEN') {
