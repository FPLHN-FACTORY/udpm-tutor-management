<template>
  <div class="shadow-xl p-3 m-3 rounded-md flex h-full flex-col overflow-auto">
    ...
    <div class="flex h-0 flex-1 flex-col">
      <tutor-table wrapperClassName="min-h-[410px]" :columns="columnsDepartment" :data-source="dataSource"
        ...
        <template #bodyCell="{ column, record }">
          ...
          <div v-if="column.key === 'action'" class="space-x-2 text-center">
            <a-tooltip title="Chi tiết lịch sử" color="#FFC26E">
              <a-button type="primary" size="regular" @click="$emit('handleOpenModalDetail', record)"
                :icon="h(EyeOutlined)" />
            </a-tooltip>
          </div>
          <div v-else-if="column.key === 'api'" class="text-center">
            <a-tooltip :title="record.api">
              <span class="ellipsis">
                {{ record.api }}
              </span>
            </a-tooltip>
          ...
          <div v-else-if="column.key === 'status'" class="text-center">
            <a-tag v-if="record.status === 'ACTIVE'" color="success">Thành công</a-tag>
            <a-tag v-else-if="record.status === 'INACTIVE'" color="error">Thất bại</a-tag>
          </div>
...
.ellipsis {
  ...
  max-width: 200px;
  /* Chỉnh kích thước tối đa theo yêu cầu */
  white-space: nowrap;
  overflow: hidden;
