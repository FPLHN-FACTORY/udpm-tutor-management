<template>
  <div class="mt-4">
    ...
    <div class="shadow-md p-3 rounded-md mx-3 mt-10">
      <div class="p-5">
        <head-subject-detail-table :data-source="planData" :loading="isLoading || isFetching"
          :pagination-params="params" :total-pages="totalPages" @update:pagination-params="handlePaginationChange"
          @handleOpenModalAddNumberTutorClass="handleOpenModalAdd"
          @handlApproveTutorClass="handlApproveTutorClass"/>
        <add-number-tutor-class :open="open" @resetTable="resetTable" @handleClose="handleClose" @cancel="open = false"
          :plan-detail="planDetail || null" :tutor-class-id="tutorClassId" />
      </div>
    </div>
  </div>
</template>
...
import HeadSubjectDetailTable from "@/pages/headsubject/HeadSubjectDetailTable.vue";
import AddNumberTutorClass from "@/pages/headsubject/AddNumberTutorClass.vue";
import { useRoute } from "vue-router";
import { computed, ref, watch, watchEffect } from "vue";
import { useDetailPlan, useGetPlanInfoById } from "@/services/service/planner/plan.action.ts";
...
import { ParamsGetTutorClass, updateStatusApproveTutorClassHeadSubject } from "@/services/api/headsubject/headsubject.api";
import { useGetTutorClass } from "@/services/service/headsubject/headsubject.action";
...
watch(userInfo, (newValue) => {
  params.value.departmentCode = newValue.departmentCode;
  params.value.facilityCode = newValue.facilityCode;
  params.value.semesterId = newValue.semesterId;
}, { immediate: true }); 

const { data, isLoading, isFetching, refetch: refetchGetTutorClass  } = useGetTutorClass(params, {
...
const { data: dataDetail, isLoading: isLoadingDetail,  refetch: refetchUseDetailPlan} = useDetailPlan(
  planId,
  {
    refetchOnWindowFocus: false,
    enabled: () => !!planId.value,
...
const handleOpenModalAdd = async (record: PlanResponse) => {
...
const handlApproveTutorClass = async (id: string) => {
  try {
    await updateStatusApproveTutorClassHeadSubject(id);
  } catch (error) {
    console.error(error);
  }
};
